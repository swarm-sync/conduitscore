...
Many site owners are confused because they think llms.txt is step one. It is not. Step one is making sure your site is crawlable, has clear author information, uses structured data, and has quality content. Those are the 46 signals that actually determine AI visibility.
...
The fix is straightforward: llms.txt is one small piece of a larger AI visibility puzzle. Before you optimize llms.txt, make sure you are strong on the 46 AI visibility signals. Then, as a refinement, add and maintain llms.txt.
...
Another common question: Will llms.txt or the 46 signals get my site into ChatGPT's training data?
...
What you can do is get your site into ChatGPT's real-time sources. When users interact with ChatGPT and ask questions, the model can pull from certain real-time sources. The 46 signals determine whether your site is a reliable source for that real-time access.
...
The 46 signals are what make you reliable for real-time citation. llms.txt is how you communicate your preferences to the systems doing the citing.
...
The bottom line: llms.txt is useful, but it is not the foundation. The 46 signals are. Start there, then layer in llms.txt.
...
The sites that will win over the next couple years are not the ones that wait for official best practices. They are the sites that understand the 46 signals, get strong across all of them, and then refine with tools like llms.txt.
...
Get the 46-Signal AI Visibility Checklist at /resources/ai-visibility-checklist for actionable steps on crawlability, structured data, and more.
